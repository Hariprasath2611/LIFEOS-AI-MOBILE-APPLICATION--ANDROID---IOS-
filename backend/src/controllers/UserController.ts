...
import { taskRepo, habitRepo, goalRepo, learningRoadmapRepo } from '../repositories';
...
export class UserController {
  ...
  async getDashboardSummary(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      // Fetch active tasks
      const allTasks = await taskRepo.findByUserId(userId);
      ...
      // Habits summary
      const habits = await habitRepo.findByUserId(userId);
      ...
      // Roadmaps
      const roadmaps = await learningRoadmapRepo.findByUserId(userId);
