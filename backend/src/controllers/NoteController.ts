...
export class NoteController {
  async getNotes(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const notes = await noteService.getNotes(req.user.uid);
      res.json(notes);
    } catch (error) {
  ...
  async getNoteById(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const { id } = req.params;
      const note = await noteService.getNoteById(req.user.uid, id);
      if (!note) {
  ...
  async createNote(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const note = await noteService.createNote(req.user.uid, req.body);
  ...
  async updateNote(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const { id } = req.params;
      const updated = await noteService.updateNote(req.user.uid, id, req.body);
      if (!updated) {
        res.status(404).json({ error: 'Note not found' });
        return;
      }
  ...
  async deleteNote(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const { id } = req.params;
      const deleted = await noteService.deleteNote(req.user.uid, id);
      if (!deleted) {
        res.status(404).json({ error: 'Note not found' });
        return;
      }
  ...
  async uploadAttachment(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const url = await uploadToCloudinary(file.buffer, 'attachments', fileName);
